...
# 期間指定（多言語対応: どの言語でも入力可能）
/canvas-collect :チェック: 過去7日
/canvas-collect :チェック: last 7 days
/canvas-collect :チェック: 过去7天
/canvas-collect :チェック: 최근 7일
/canvas-collect :チェック: derniers 7 jours
/canvas-collect :チェック: últimos 7 días
/canvas-collect :チェック: पिछले 7 दिन

# 組み合わせ（有効）
/canvas-collect :チェック: #マーケティング 過去30日
/canvas-collect :チェック: #営業 #開発 last 7 days
```
...
#### Canvas名
- **形式**: `[絵文字] Collection Log`（全言語共通・英語固定）
- **例**: `:チェック: Collection Log`、`:重要: Collection Log`
- **理由**: Canvasはチャンネル共有リソースのため、タイトル一致検索の整合性を保証
...
| 項目 | 方針 |
|------|------|
| 言語検出 | Slack `users.info` API（`include_locale: true`）でユーザーlocale取得 |
| キャッシュ | 30分TTLのインメモリキャッシュで `users.info` 呼び出しを最小化 |
| Canvas タイトル | **英語固定** `:emoji: Collection Log`（検索整合性のため） |
| Canvas コンテンツ | ユーザーlocaleでローカライズ |
| エフェメラルメッセージ | ユーザーlocaleでローカライズ |
| i18nライブラリ | 外部ライブラリ不使用（~34文字列のため `{{variable}}` プレースホルダー方式で自作） |
| 型安全 | TypeScript `Messages` interfaceで全localeファイルのキー漏れをコンパイル時検出 |

#### コマンド構文ローカライズ

期間指定（`過去○日`）を全7言語でパース可能にする。全パターンをOR結合した1つの正規表現で一括マッチ（localeに関係なくどの言語でも入力OK）。
...
#### エラー伝播パターン
- `slack-api.ts` のエラーは `messageKey`（i18nキー）を `AppError` に格納し、catch側で翻訳
- `command-parser.ts` のエラーは `locale` を受け取り、throw時に翻訳済みメッセージをセット

#### 追加スコープ
- `users:read`（ユーザーlocale取得に必要）
...
| 1.5 | 2026-02-08 | **フェーズ4.5 i18n実装**。7言語対応（ja/en/hi/fr/es/zh/ko）、Canvas名英語固定、多言語期間パース、`users:read`/`chat:write`スコープ追加。 |
